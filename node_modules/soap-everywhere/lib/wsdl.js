...
function splitNSName(nsName) {
  ...
  return i < 0
    ...
    : { namespace: nsName.substring(0, i), name: nsName.substring(i + 1) };
}
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    ...
    this.element = definitions.schemas[definitions.xmlns[ns]].elements[
      nsName.name
    ];
    ...
    for (var i = 0, part; (part = this.children[i]); i++) {
      ...
      if (typeof schemaDefinition !== "undefined") {
        this.parts[part.$name] = definitions.schemas[ns].types[type] ||
...
WSDL.prototype.xmlToObject = function(xml) {
  var self = this,
    ...
    pHandler,
    p = new xmlParser.SaxParser(function(cb) {
      pHandler = cb;
    }),
    objectName = null,
    root = {},
  ...
  pHandler.onStartElementNS(function(nsName, attrs, prefix, uri, namespaces) {
    ...
    if (!objectName && top.name === "Body" && name !== "Fault") {
      ...
      if (!message) {
        ...
        self.definitions.messages[originalName] = self.definitions.messages[
          name
        ];
      }
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
  ...
  for (var key in params) {
    if (key != nsAttrName) {
      ...
      parts.push(
        typeof value === "object" ? this.objectToXML(value) : xmlEscape(value)
      );
      parts.push(["</", namespace, ":", key, ">"].join(""));
    }
  }
